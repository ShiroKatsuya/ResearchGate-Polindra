...
<!--
  ...
  Catatan: Mode gelap tersedia sebagai opsi tetapi nonaktif secara default.
-->
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 text-gray-900 antialiased font-sans selection:bg-primary/20 selection:text-primary-800 smooth-scroll">
  <section class="max-w-4xl w-full mx-auto py-12">
    <div class="grid w-full gap-8 items-center lg:grid-cols-2">
      <div x-data="loginForm()" x-init="init()" class="bg-white dark:bg-gray-800 shadow-2xl rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 transform transition-all duration-300 hover:scale-[1.005]">
        <!-- Header / Logo -->
        <header class="px-6 py-6 sm:py-8 flex items-center justify-between bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
          <div class="flex items-center gap-3">
            ...
            <svg class="w-12 h-12 rounded-full p-2 bg-white bg-opacity-20 text-white shadow-inner" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
          ...
          <!-- Dark mode toggle (nonaktif by default) -->
          <div class="flex items-center gap-2">
            <button type="button" @click="toggleDark()" class="group p-2 rounded-full focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-300 text-white hover:bg-white hover:bg-opacity-20 transition-colors" :aria-pressed="isDark.toString()">
              <svg x-show="!isDark" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m8.66-11.66l-.7.7M4.34 19.66l-.7.7M21 12h-1M4 12H3m15.66 5.66l-.7-.7M6.34 4.34l-.7-.7" />
              </svg>
              <svg x-show="isDark" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-yellow-300" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                <path d="M10 2a1 1 0 00-.894.553 6 6 0 108.341 8.341A1 1 0 0018 10 8 8 0 0110 2z" />
              </svg>
        ...
        <div class="px-6 pb-6 sm:pb-8 pt-6">
          <form @submit.prevent="submit()" class="space-y-5">
            ...
            <div class="flex items-center justify-between">
              <label class="text-sm font-semibold text-gray-800 dark:text-gray-200">Masuk</label>
            ...
            <!-- Username / Email -->
            <div>
              <label for="identity" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Email / Username</label>
              <div class="mt-1 relative">
                <input id="identity" name="identity" type="text" x-model="form.identity" placeholder="contoh@contoh.com atau username" aria-describedby="identity-note identity-error" aria-invalid="false" class="w-full px-4 py-2 pl-10 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200" />
            ...
            <div>
              <label for="password" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Kata Sandi</label>
              <div class="mt-1 relative">
                <input id="password" :type="showPassword ? 'text' : 'password'" x-model="form.password" name="password" aria-describedby="password-error" aria-invalid="false" placeholder="Masukkan kata sandi" class="w-full px-4 py-2 pl-10 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 pr-12 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200" />
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                ...
                <button type="button" @click="showPassword = !showPassword" :aria-pressed="showPassword.toString()" class="absolute right-2 top-1/2 -translate-y-1/2 p-2 rounded-full text-gray-500 hover:bg-gray-200 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors" aria-label="Tampilkan / Sembunyikan kata sandi">
            ...
            <div class="flex items-center justify-between">
              <label class="inline-flex items-center text-sm">
                <input type="checkbox" x-model="form.remember" class="h-4 w-4 rounded text-indigo-600 focus:ring-indigo-500 border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700" />
                <span class="ml-2 text-gray-700 dark:text-gray-200">Ingat saya</span>
              </label>
            ...
            <div>
              <button :disabled="loading" :class="loading ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-md active:scale-[0.98] transition-all duration-200'" type="submit" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg font-semibold focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 shadow-lg">
            ...
            <div class="grid grid-cols-2 gap-3">
              <button type="button" @click="social('google')" class="inline-flex items-center justify-center gap-2 px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 active:scale-[0.98] transition-all duration-200">
                <!-- Google SVG small -->
              ...
              <button type="button" @click="social('github')" class="inline-flex items-center justify-center gap-2 px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-indigo-500 text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 active:scale-[0.98] transition-all duration-200">
                <!-- GitHub Icon -->
        ...
        <!-- Footer (policy) -->
        <footer class="px-6 py-4 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-500 text-center">
        ...
        <div x-show="forgotOpen" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-40 p-4">
          <div @click.away="forgotOpen = false" class="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl p-6 w-full max-w-md shadow-lg border border-gray-100 dark:border-gray-700 transform transition-all duration-300">
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">Lupa Kata Sandi</h3>
              <button @click="forgotOpen = false" class="p-2 rounded-full text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors">
                <span class="sr-only">Tutup</span>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
              </button>
            </div>
            <p class="mt-2 text-sm text-gray-500 dark:text-gray-300">Masukkan email Anda untuk menerima instruksi pemulihan (demo saja).</p>
            <div class="mt-4">
              <label for="fp-email" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Email</label>
              <input id="fp-email" type="email" class="mt-1 w-full px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200" placeholder="email@contoh.com">
            </div>
            <div class="mt-5 flex justify-end gap-3">
              <button @click="forgotOpen=false" class="px-4 py-2 rounded-lg text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-colors">Batal</button>
        ...
        <div aria-live="polite" class="fixed inset-0 flex items-end px-4 py-6 pointer-events-none sm:p-6 z-50">
          <div class="w-full flex flex-col items-center sm:items-end space-y-4">
            <div x-show="toast.show" x-cloak x-transition:enter="transform ease-out duration-300 transition" x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-text="toast.message" class="pointer-events-auto max-w-sm w-full bg-white dark:bg-gray-800 shadow-lg rounded-lg border border-gray-100 dark:border-gray-700 px-4 py-3 text-sm text-gray-800 dark:text-gray-100"></div>
          </div>
        </div>
      </div>
      <div class="hidden lg:block">
        <div class="relative isolate overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-indigo-50 via-violet-50 to-sky-50 p-10 shadow-sm dark:border-slate-800 dark:from-slate-900 dark:via-indigo-950 dark:to-slate-900">
          <div class="absolute -left-10 -top-10 h-56 w-56 rounded-full bg-indigo-300/20 blur-3xl dark:bg-indigo-500/10"></div>
          <div class="absolute -bottom-16 -right-16 h-64 w-64 rounded-full bg-sky-300/20 blur-3xl dark:bg-sky-500/10"></div>
          <div class="relative">
            <h2 class="mb-3 text-2xl font-semibold tracking-tight text-slate-800 dark:text-slate-100">Selamat datang kembali 👋</h2>
            <p class="max-w-md text-slate-600 dark:text-slate-300">Masuk untuk melanjutkan eksplorasi riset, proyek, dan publikasi terbaru. Keamanan akun Anda adalah prioritas kami.</p>
            <ul class="mt-6 space-y-2 text-sm text-slate-600 dark:text-slate-300">
              ...
              <li class="flex items-start gap-2"><svg xmlns="http://www.w3.org/2000/svg" class="mt-0.5 h-5 w-5 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"></path></svg><span>Mode gelap yang nyaman untuk mata.</span></li>
            </ul>
          </div>
        </div>
      </div>
  ...
  function loginForm(){
    return {
      ...
      toggleDark(){
        ...
        localStorage.setItem('dark-mode', this.isDark); // Save preference
      },
