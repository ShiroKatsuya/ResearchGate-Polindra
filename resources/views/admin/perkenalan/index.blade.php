...
<div class="min-h-screen bg-gradient-to-br py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @include('navigation_perkenalan')
        
        <!-- Header Section -->
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Perkenalan</h1>
                    <p class="text-gray-600">Kelola data perkenalan mahasiswa/alumni</p>
                </div>
                <a href="{{ route('perkenalan.create') }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" action="{{ route('perkenalan.index') }}" id="searchForm" class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-blue-800 font-medium">
                        Hasil Pencarian: {{ $perkenalans->total() }} data ditemukan
                    </span>
                </div>
                <a href="{{ route('perkenalan.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    Lihat Semua Perkenalan
                </a>
            </div>
        </div>
        @endif
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150 flex flex-col md:table-row md:flex-row w-full md:w-auto">
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Nama">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $perkenalan->name }}</div>
                                        <div class="text-xs text-gray-500">Dibuat: {{ $perkenalan->created_at ? \Carbon\Carbon::parse($perkenalan->created_at)->format('d M Y') : '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Program Studi">
                                <div class="text-sm text-gray-900">{{ $perkenalan->program_study ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Tahun Lulus">
                                <div class="text-sm text-gray-900">{{ $perkenalan->year_of_graduation ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Email">
                                <div class="text-sm text-gray-900 break-all">{{ $perkenalan->email ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Telepon">
                                <div class="text-sm text-gray-900 break-all">{{ $perkenalan->phone ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium flex flex-col md:table-cell" data-label="Aksi">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('perkenalan.edit', $perkenalan->id) }}" 
                                    ...
                                    <button type="button" 
                                            onclick="openDeleteModal({{ $perkenalan->id }}, '{{ addslashes($perkenalan->name) }}')"
                            ...
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <p class="text-lg font-medium">
                                        @if(request('search') || request('start_date') || request('end_date'))
                                            Tidak ada data perkenalan yang ditemukan
                                        @else
                                            Belum ada data perkenalan
                                        @endif
                                    </p>
                                    <p class="text-sm">
                                        @if(request('search') || request('start_date') || request('end_date'))
                                        ...
                                        @else
                                            Mulai dengan menambahkan perkenalan pertama Anda
                                        @endif
                                    </p>
                                </div>
                            </td>
            ...
            <div class="bg-white px-6 py-6 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        <span>
                            Showing <span class="font-semibold text-gray-900">{{ $perkenalans->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $perkenalans->lastItem() }}</span> of <span class="font-semibold text-gray-900">{{ $perkenalans->total() }}</span> results
                        </span>
                    </div>

                    <div class="flex items-center space-x-1">
                        @if($perkenalans->onFirstPage())
                        ...
                        @else
                        <a href="{{ $perkenalans->previousPageUrl() }}" 
                        ...
                        <div class="flex items-center space-x-1">
                            @foreach($perkenalans->getUrlRange(1, $perkenalans->lastPage()) as $page => $url)
                                @if($page == $perkenalans->currentPage())
                                ...
                                @elseif($page == 1 || $page == $perkenalans->lastPage() || ($page >= $perkenalans->currentPage() - 1 && $page <= $perkenalans->currentPage() + 1))
                                <a href="{{ $url }}" 
                                   class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
                                    {{ $page }}
                                </a>
                                @elseif($page == $perkenalans->currentPage() - 2 || $page == $perkenalans->currentPage() + 2)
                        ...
                        @if($perkenalans->hasMorePages())
                        <a href="{{ $perkenalans->nextPageUrl() }}" 
                           class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
                            Next
                    ...
                    <div class="flex sm:hidden items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-600">
                            Page {{ $perkenalans->currentPage() }} of {{ $perkenalans->lastPage() }}
                        </span>
        ...
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl ring-1 ring-gray-900/5 w-full sm:max-w-lg transition-all duration-300 scale-95 opacity-0" id="modalContent">
            ...
            <div class="px-6 py-6 space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-gray-900 font-medium">Anda yakin ingin menghapus perkenalan ini?</p>
