    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Publikasi</h1>
                    <p class="text-gray-600">Kelola data publikasi penelitian atau karya ilmiah</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" action="{{ route('publikasi.index') }}" id="searchForm" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div class="relative">
                        <select name="journal" id="journalSelect" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200">
                            <option value="">Semua Jurnal</option>
                            @foreach($journals as $journal)
                            <option value="{{ $journal }}" {{ request('journal') == $journal ? 'selected' : '' }}>
                                {{ $journal }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex space-x-2">
                        ...
                        @if(request('search') || request('journal') || request('published_date'))
            ...
            <div class="mt-4 pt-4 border-t border-gray-200">
                <div class="flex items-center flex-wrap gap-2">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm bg-purple-100 text-purple-800">
                        ...
                        Tanggal: {{ \Carbon\Carbon::parse(request('published_date'))->format('d M Y') }}
                    </span>
                    @endif
                </div>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-blue-800 font-medium">
                        Hasil Pencarian: {{ $publikasis->total() }} publikasi ditemukan
                    </span>
                </div>
                <a href="{{ route('publikasi.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                ...
                <table class="min-w-full divide-y divide-gray-200 mb-4 md:mb-0">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150 flex flex-col md:table-row md:flex-row w-full md:w-auto">
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Publikasi">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $publikasi->title }}</div>
                                        @if($publikasi->doi)
                                        <div class="text-sm text-gray-500">DOI: {{ $publikasi->doi }}</div>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Jurnal">
                                <div class="text-sm text-gray-900">{{ $publikasi->journal ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Mahasiswa">
                                <div class="text-sm text-gray-900">
                                    {{ $publikasi->student ? $publikasi->student->name : '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Tanggal Publikasi">
                                <div class="text-sm text-gray-900">
                                    {{ $publikasi->published_at ? $publikasi->published_at->format('d M Y') : '-' }}
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-right text-sm font-medium flex flex-col md:table-cell" data-label="Aksi">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button type="button" 
                                            onclick="openDeleteModal({{ $publikasi->id }}, '{{ addslashes($publikasi->title) }}')"
                            ...
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <p class="text-sm">
                                        ...
                                        @else
                                            Mulai dengan menambahkan publikasi pertama Anda
                                        @endif
                                    </p>
                                </div>
                            </td>
            ...
            <div class="bg-white px-6 py-6 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        <span>
                            Showing <span class="font-semibold text-gray-900">{{ $publikasis->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $publikasis->lastItem() }}</span> of <span class="font-semibold text-gray-900">{{ $publikasis->total() }}</span> results
                        </span>
                    </div>
                    ...
                    <div class="flex items-center space-x-1">
                        ...
                        @else
                        <a href="{{ $publikasis->previousPageUrl() }}" 
                        ...
                        <div class="flex items-center space-x-1">
                            @foreach($publikasis->getUrlRange(1, $publikasis->lastPage()) as $page => $url)
                                ...
                                @elseif($page == 1 || $page == $publikasis->lastPage() || ($page >= $publikasis->currentPage() - 1 && $page <= $publikasis->currentPage() + 1))
                                <a href="{{ $url }}" 
                                   class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
                                    {{ $page }}
                                </a>
                                @elseif($page == $publikasis->currentPage() - 2 || $page == $publikasis->currentPage() + 2)
                    ...
                    <div class="flex sm:hidden items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-600">
                            Page {{ $publikasis->currentPage() }} of {{ $publikasis->lastPage() }}
                        </span>
...
<div id="deleteModal" class="fixed inset-0 z-50 overflow-y-auto hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
        <div class="fixed inset-0 bg-opacity-75 transition-opacity" onclick="closeDeleteModal()"></div>
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl ring-1 ring-gray-900/5 w-full sm:max-w-lg transition-all duration-300 scale-95 opacity-0" id="modalContent">
            ...
            <div class="px-6 py-6 space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-gray-900 font-medium">Anda yakin ingin menghapus publikasi ini?</p>
                ...
                <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-red-700 text-sm mt-1">Data yang dihapus tidak dapat dipulihkan. Pastikan Anda telah memilih publikasi yang benar.</p>
                        </div>
                    </div>
                </div>
            </div>
