    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Edit Proyek</h1>
                    <p class="text-gray-600">Perbarui informasi proyek penelitian mahasiswa</p>
                </div>
            ...
            <form action="{{ route('proyek.update', $proyek->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea id="description" 
                              ...
                              placeholder="Masukkan deskripsi proyek">{{ old('description', $proyek->description) }}</textarea>
                    @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select id="status" name="status" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('status') border-red-500 @enderror">
                        @php
                            $currentStatus = old('status', $proyek->status);
                        @endphp
                        ...
                        <option value="tertunda" {{ $currentStatus == 'tertunda' ? 'selected' : '' }}>Tertunda</option>
                    </select>
                    @error('status')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="date" 
                               id="start_date" 
                               name="start_date" 
                               value="{{ old('start_date', $proyek->start_date ? \Carbon\Carbon::parse($proyek->start_date)->format('Y-m-d') : '') }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('start_date') border-red-500 @enderror">
                        @error('start_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div>
                        ...
                        <input type="date" 
                               id="end_date" 
                               name="end_date" 
                               value="{{ old('end_date', $proyek->end_date ? \Carbon\Carbon::parse($proyek->end_date)->format('Y-m-d') : '') }}"
                               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('end_date') border-red-500 @enderror">
                        @error('end_date')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <input type="url" 
                           id="repository_url" 
                           name="repository_url" 
                           value="{{ old('repository_url', $proyek->repository_url) }}"
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 @error('repository_url') border-red-500 @enderror"
                ...
                <div>
                    ...
                    <input type="hidden" id="student_id" name="student_id" value="{{ old('student_id', $proyek->student_id) }}">
