...
<div class="min-h-screen bg-gradient-to-br py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-600">Kelola data berita dan pengumuman</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" action="{{ route('berita.index') }}" id="searchForm" class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-blue-800 font-medium">
                        Hasil Pencarian: {{ $beritas->total() }} data ditemukan
                    </span>
                </div>
                <a href="{{ route('berita.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    Lihat Semua Berita
                </a>
            </div>
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200 mb-4 md:mb-0">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150 flex flex-col md:table-row md:flex-row w-full md:w-auto">
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Judul">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-xs text-gray-500">Terbit: {{ $berita->published_at ? \Carbon\Carbon::parse($berita->published_at)->format('d M Y') : '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Slug">
                                <div class="text-sm text-gray-900 break-all">{{ $berita->slug ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Kategori">
                                <div class="text-sm text-gray-900">{{ optional($berita->category)->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Tanggal Terbit">
                                <div class="text-sm text-gray-900">{{ $berita->published_at ? \Carbon\Carbon::parse($berita->published_at)->format('d M Y') : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium flex flex-col md:table-cell" data-label="Aksi">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button type="button" 
                                            onclick="openDeleteModal({{ $berita->id }}, '{{ addslashes($berita->title) }}')"
            ...
            <div class="bg-white px-6 py-6 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        <span>
                            Showing <span class="font-semibold text-gray-900">{{ $beritas->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $beritas->lastItem() }}</span> of <span class="font-semibold text-gray-900">{{ $beritas->total() }}</span> results
                        </span>
                    </div>
                    ...
                    <div class="flex items-center space-x-1">
                        ...
                        <div class="flex items-center space-x-1">
                            @foreach($beritas->getUrlRange(1, $beritas->lastPage()) as $page => $url)
                                ...
                                @elseif($page == 1 || $page == $beritas->lastPage() || ($page >= $beritas->currentPage() - 1 && $page <= $beritas->currentPage() + 1))
                                <a href="{{ $url }}" 
                                   class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
                                    {{ $page }}
                                </a>
                                @elseif($page == $beritas->currentPage() - 2 || $page == $beritas->currentPage() + 2)
        ...
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl ring-1 ring-gray-900/5 w-full sm:max-w-lg transition-all duration-300 scale-95 opacity-0" id="modalContent">
            ...
            <div class="px-6 py-6 space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-gray-900 font-medium">Anda yakin ingin menghapus berita ini?</p>
                ...
                <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-red-700 text-sm mt-1">Data yang dihapus tidak dapat dipulihkan. Pastikan Anda telah memilih berita yang benar.</p>
                        </div>
                    </div>
                </div>
            </div>
