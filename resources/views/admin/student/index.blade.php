...
<div class="min-h-screen bg-gradient-to-br py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @include('navigation_student')
        
        <!-- Header Section -->
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Mahasiswa</h1>
                    <p class="text-gray-600">Kelola data mahasiswa/alumni</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" action="{{ route('student.index') }}" id="searchForm" class="space-y-4">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-blue-800 font-medium">
                        Hasil Pencarian: {{ $students->total() }} data ditemukan
                    </span>
                </div>
                <a href="{{ route('student.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150 flex flex-col md:table-row md:flex-row w-full md:w-auto">
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Nama">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        <div class="text-xs text-gray-500">Dibuat: {{ $student->created_at ? \Carbon\Carbon::parse($student->created_at)->format('d M Y') : '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Program Studi">
                                <div class="text-sm text-gray-900">{{ $student->program_study ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Tahun Lulus">
                                <div class="text-sm text-gray-900">{{ $student->year_of_graduation ?? '-' }}</div>
                            </td>
                            ...
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Telepon">
                                <div class="text-sm text-gray-900 break-all">{{ $student->phone ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium flex flex-col md:table-cell" data-label="Aksi">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <button type="button" 
                                            onclick="openDeleteModal({{ $student->id }}, '{{ addslashes($student->name) }}')"
            ...
            <div class="bg-white px-6 py-6 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        <span>
                            Showing <span class="font-semibold text-gray-900">{{ $students->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $students->lastItem() }}</span> of <span class="font-semibold text-gray-900">{{ $students->total() }}</span> results
                        </span>
                    </div>
        ...
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl ring-1 ring-gray-900/5 w-full sm:max-w-lg transition-all duration-300 scale-95 opacity-0" id="modalContent">
            ...
            <div class="px-6 py-6 space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-gray-900 font-medium">Anda yakin ingin menghapus data mahasiswa ini?</p>
                ...
                <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-red-700 text-sm mt-1">Data yang dihapus tidak dapat dipulihkan. Pastikan Anda telah memilih data mahasiswa yang benar.</p>
                        </div>
                    </div>
                </div>
            </div>
