...
<div class="min-h-screen bg-gradient-to-br py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @include('navigation_perangkatlunak')
        
        <!-- Header Section -->
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 mb-2">Manajemen Perangkat Lunak</h1>
                    <p class="text-gray-600">Kelola data perangkat lunak mahasiswa</p>
                </div>
                <a href="{{ route('perangkatlunak.create') }}" 
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" action="{{ route('perangkatlunak.index') }}" id="searchForm" class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-6 gap-4">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if(request('search') || request('student_name') || request('start_date') || request('end_date'))
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center">
                    ...
                    <span class="text-blue-800 font-medium">
                        Hasil Pencarian: {{ $perangkatlunaks->total() }} perangkat lunak ditemukan
                    </span>
                </div>
                <a href="{{ route('perangkatlunak.index') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
                    Lihat Semua Perangkat Lunak
                </a>
            </div>
        </div>
        @endif
            ...
            <div class="overflow-x-auto mb-4 md:mb-0">
                <table class="min-w-full divide-y divide-gray-200 mb-4 md:mb-0">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($perangkatlunaks as $perangkatlunak)
                        <tr class="hover:bg-gray-50 transition-colors duration-150 flex flex-col md:table-row md:flex-row w-full md:w-auto">
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Perangkat Lunak">
                                <div class="flex items-start">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">{{ $perangkatlunak->name }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Mahasiswa">
                                <div class="text-sm text-gray-900">
                                    {{ $perangkatlunak->student ? $perangkatlunak->student->name : '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Repo">
                                <div class="text-sm text-gray-900 break-all">
                                    @if($perangkatlunak->repo_url)
                                        <a href="{{ $perangkatlunak->repo_url }}" target="_blank" class="text-blue-600 hover:underline">{{ $perangkatlunak->repo_url }}</a>
                                    @else
                                        -
                                    @endif
                                </div>
                            ...
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Website">
                                <div class="text-sm text-gray-900 break-all">
                                    @if($perangkatlunak->website_url)
                                        <a href="{{ $perangkatlunak->website_url }}" target="_blank" class="text-blue-600 hover:underline">{{ $perangkatlunak->website_url }}</a>
                                    @else
                                        -
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 flex flex-col md:table-cell" data-label="Dibuat">
                                <div class="text-sm text-gray-900">
                                    {{ $perangkatlunak->created_at ? \Carbon\Carbon::parse($perangkatlunak->created_at)->format('d M Y') : '-' }}
                                </div>
                            </td>
                            <td class="px-6 py-4 text-right text-sm font-medium flex flex-col md:table-cell" data-label="Aksi">
                                <div class="flex items-center justify-end space-x-2">
                                    <a href="{{ route('perangkatlunak.edit', $perangkatlunak->id) }}" 
                                    ...
                                    <button type="button" 
                                            onclick="openDeleteModal({{ $perangkatlunak->id }}, '{{ addslashes($perangkatlunak->name) }}')"
                            ...
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="text-gray-500">
                                    ...
                                    <p class="text-lg font-medium">
                                        @if(request('search') || request('student_name') || request('start_date') || request('end_date'))
                                            Tidak ada perangkat lunak yang ditemukan
                                        @else
                                    ...
                                    <p class="text-sm">
                                        ...
                                        @else
                                            Mulai dengan menambahkan perangkat lunak pertama Anda
                                        @endif
                                    </p>
                                </div>
                            </td>
            ...
            <div class="bg-white px-6 py-6 border-t border-gray-200">
                <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div class="flex items-center text-sm text-gray-600">
                        ...
                        <span>
                            Showing <span class="font-semibold text-gray-900">{{ $perangkatlunaks->firstItem() }}</span> to <span class="font-semibold text-gray-900">{{ $perangkatlunaks->lastItem() }}</span> of <span class="font-semibold text-gray-900">{{ $perangkatlunaks->total() }}</span> results
                        </span>
                    </div>

                    <div class="flex items-center space-x-1">
                        @if($perangkatlunaks->onFirstPage())
                        ...
                        @else
                        <a href="{{ $perangkatlunaks->previousPageUrl() }}" 
                        ...
                        <div class="flex items-center space-x-1">
                            @foreach($perangkatlunaks->getUrlRange(1, $perangkatlunaks->lastPage()) as $page => $url)
                                @if($page == $perangkatlunaks->currentPage())
                                ...
                                @elseif($page == 1 || $page == $perangkatlunaks->lastPage() || ($page >= $perangkatlunaks->currentPage() - 1 && $page <= $perangkatlunaks->currentPage() + 1))
                                <a href="{{ $url }}" 
                                   class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
                                    {{ $page }}
                                </a>
                                @elseif($page == $perangkatlunaks->currentPage() - 2 || $page == $perangkatlunaks->currentPage() + 2)
                        ...
                        @if($perangkatlunaks->hasMorePages())
                        <a href="{{ $perangkatlunaks->nextPageUrl() }}" 
                           class="relative inline-flex items-center px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 hover:text-gray-900 transition-all duration-200">
                            Next
                    ...
                    <div class="flex sm:hidden items-center space-x-2">
                        ...
                        <span class="text-sm text-gray-600">
                            Page {{ $perangkatlunaks->currentPage() }} of {{ $perangkatlunaks->lastPage() }}
                        </span>
        ...
        <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl ring-1 ring-gray-900/5 w-full sm:max-w-lg transition-all duration-300 scale-95 opacity-0" id="modalContent">
            ...
            <div class="px-6 py-6 space-y-4">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-gray-900 font-medium">Anda yakin ingin menghapus perangkat lunak ini?</p>
                ...
                <div class="bg-red-50 border border-red-200 rounded-lg p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-red-700 text-sm mt-1">Data yang dihapus tidak dapat dipulihkan. Pastikan Anda telah memilih perangkat lunak yang benar.</p>
                        </div>
                    </div>
                </div>
            </div>
